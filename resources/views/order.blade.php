    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="glass-panel rounded-xl p-6 sticky top-24">
                <img src="{{ $game->thumbnail ?? 'https://placehold.co/600x400/1e293b/FFF?text='.urlencode($game->name) }}" alt="{{ $game->name }}" class="w-32 h-32 rounded-xl shadow-lg mb-4 mx-auto lg:mx-0 object-cover">
            ...
            <form action="{{ route('order.process') }}" method="POST" id="orderForm">
                ...
                <div class="glass-panel rounded-xl p-6 mb-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <input type="text" name="target_id" required placeholder="User ID" class="bg-gray-800/10 border border-slate-600 rounded-lg p-3 w-full text-white">
                        <input type="text" name="server_id" placeholder="Server ID (Optional)" class="bg-gray-800/10 border border-slate-600 rounded-lg p-3 w-full text-white">
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        @foreach($game->items as $item)
                            <label class="cursor-pointer">
                                ...
                                <div class="bg-gray-800/10 border border-slate-600 rounded-lg p-4 peer-checked:border-blue-500 peer-checked:bg-blue-900/20 hover:bg-gray-800/30 transition">
                                    <p class="font-bold text-white">{{ $item->name }}</p>
                                    <p class="text-violet-400 text-sm">Rp {{ number_format($item->price, 0, ',', '.') }}</p>
                                </div>
                            </label>
                        @endforeach
                    </div>
                    ...
                    <div class="space-y-3">
                        @foreach($payments as $payment)
                            <label class="cursor-pointer block">
                                ...
                                <div class="bg-gray-800/10 border border-slate-600 rounded-lg p-4 flex items-center justify-between peer-checked:border-blue-500 peer-checked:bg-blue-900/20 hover:bg-gray-800/30">
                ...
                <!-- Tombol Beli -->
                <button type="submit" class="w-full bg-violet-600 hover:bg-violet-500 text-white font-bold py-4 rounded-xl shadow-lg transition transform hover:scale-[1.02]">
